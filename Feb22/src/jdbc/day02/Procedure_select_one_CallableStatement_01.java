package jdbc.day02;
//프로시저를 사용하는 이유는 컴파일을 하면 이미 문법검사는 끝맻히고 변하는 값만 대입하기 때문에 속도 측면에서 굉장히 향상된다.

/*
 			== 먼저 jdbc_day02.sql 파일을 열어서 tbl_class 테이블 및 tbl_student 테이블을 생성한다.
			== 먼저 아래와 같이 오라클에서 프로시저를 생성해야 한다.==
...
try {
			...
			// 이부분 정확히 기억하기 
			cstmt = conn.prepareCall(" {call pcd_student_select_one(?,?,?,?,?,?,?)} ");  //위치홀더 사용
			// return 타입은 CallableStatement
			/*
			...
			*/
			...
			// 첫번째만 IN 모드 나머지는 OUT모드 
			cstmt.setString(1, stno);  // 숫자 1 은 프로시저 파라미터중 첫번째 파라미터인 IN 모드의 ?(위치홀더) 를 말한다.
			cstmt.registerOutParameter(2, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 2는 프로시저 파라미터중 두번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다.  
			cstmt.registerOutParameter(3, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 3는 프로시저 파라미터중 세번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다. 
			cstmt.registerOutParameter(4, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 4는 프로시저 파라미터중 네번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다. 
			cstmt.registerOutParameter(5, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 5는 프로시저 파라미터중 다섯번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다. 
			cstmt.registerOutParameter(6, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 6는 프로시저 파라미터중 여섯번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다. 
			cstmt.registerOutParameter(7, java.sql.Types.VARCHAR);  // OUT 모드의 타입을 변경(문자열)  // 숫자 7는 프로시저 파라미터중 일곱번째 파라미터인 OUT 모드의 ?(위치홀더)를 말한다. 
			
			
			
			// >>> 4. CallableStatement cstmt 객체를 사용하여 오라클의 프로시저 실행하기 <<< //
			cstmt.executeUpdate();   //insert여도 이것으로 한다.  // 오라클 서버에게 해당 프로시저를 실행해라는 것이다.
			// 프로시저의 실행은 cstmt.executeUpdate(); 또는 cstmt.execute(); 이다
			
			if( cstmt.getString(2) == null)   // 2번째 위치 홀더에 null이 들어간다면 
			{
				System.out.println(">>> 입력하신 학번은 " + stno + "존재하지 않습니다. <<<");
			}
			else
			{
				System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
				...
				// 위의 cstmt.getString(2)에서 숫자 2는 프로시저 파라미터 중 두번째 파라미터인 OUT 모드의 결과값을 말한다.
				// 나머지 3 부터 7도 동일한 것이다.
		...
		// 반납하기 
			...
			try {
				if(cstmt != null)
				{
					cstmt.close();   //null 이 존재하기에 null 이 아니면 정상 종료
					cstmt = null;
				}
				if(conn != null)
				{
